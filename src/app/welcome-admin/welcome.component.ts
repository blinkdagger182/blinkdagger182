...
export class WelcomeComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        setTimeout(() => {
            // this.divClick.nativeElement.click();  // comment for remove error, later will check
        }, 200);
    ...
    loginType = 'admin'; iamadmin = false;
    loginSubmit() {
        this.loading = true;
        ...
        this._authService.login(username, password, this.loginType).subscribe(
            data => {
                ...
                //this.document.location.href = '/admin/index';

                let usrLoginLvl = JSON.parse(localStorage.getItem('currentUser'));
                if ((usrLoginLvl) || (usrLoginLvl.job_role !== null)) {

                    if(usrLoginLvl.job_role == '9' || usrLoginLvl.job_role == '10')
                        this.dashUrl = '/admin/maps';
                    else
                        this.dashUrl = '/admin/index';

                }

                this.returnUrl = this._route.snapshot.queryParams['returnUrl'] || this.dashUrl;
