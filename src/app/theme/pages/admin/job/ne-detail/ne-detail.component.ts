...
import { JDVars } from './ne-detail-vars';
...
@Injectable()
export class NEDetailComponent implements OnInit, AfterViewInit {
    ...
    getJobDetailData() {
        ...
        this.getJobDetail().subscribe(data => {
            ...
            this.advPosForm.setValue({
                ...
                advPosRequestor: '', advPosApprover: '',
            });
            ...
            console.log('')
            // not authorized - HCBO only
            if ((roleArr.indexOf('3') >= 0)) {
                this.canUpdPosDesc = true;
                if (this.data.profile[0].ablePromotion != 1) {
    ...
    ngOnInit() {
        ...
        this.advPosForm = new FormGroup({
            ...
            advPosRequestor: new FormControl(), advPosApprover: new FormControl(),
        });
    }

    ngAfterViewInit() {
        this._script.loadScripts('app-ne-detail',
            [
    ...
    onadvPosFormSubmit(): void {
        ...
        let posRemark = $("#advertise_remarks").val().toString();
        ...
        //console.log(this.dataAdvPos);
        /**** REMARK SEKEJAP TODO NANTI REMOVE   */
    ...
    optAppList2 = [];
    getApprover() {
        this._GET_api_Service.GET_data(this.approverListAPI).subscribe(data => {
    ...
    compareTwoDates() { //this.advPosForm.get('advPosIdx2').value;
        ...
        this.advDateError = {
            isError: isErr, errorMessage: errMsg,
        };
