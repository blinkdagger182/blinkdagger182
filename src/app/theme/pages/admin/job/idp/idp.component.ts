...
export class idpComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.filterForm = new FormGroup({
            idp_year: new FormControl('', Validators.required),
            status: new FormControl('', Validators.required),
            persno: new FormControl('', Validators.required),
            reptto: new FormControl('', Validators.required),
            empstatus: new FormControl('0', Validators.required),
    ...
    ngAfterViewInit() {
        this._script.loadScripts('app-idp',
            [
    ...
    checkLevel() {
        let usrRole = this.currentUser.job_role;
    ...
    batchInfo;
    getBatch() {
        this._GET_api_Service.GET_IDP_data(INVars.getBatchAPI).subscribe(data => {
            this.batchInfo = data;
            let stat = this.batchInfo.find(x => x.idp_year == this.currYr);
            if (stat) {
                this.filterForm.patchValue({ idp_year: this.currYr });
            }
        }, error => {
            console.log('[ERROR - Fail to get IDP batches] ' + error);
        });
    }

    lobInfo;
    showAllLob: Boolean = false;
    getLob() {
        this.loading = true;
        let usrRole = this.currentUser.job_role;
        this._GET_api_Service.GET_IDP_data(INVars.getLobAPI).subscribe(data => {
            if ((/1/i.test(usrRole)) || (/6/i.test(usrRole))) {
                this.showAllLob = true;
                this.lobInfo = data.lob;
                this.loading = false;
            } else {
                this.lobInfo = data.default;
                if (this.lobInfo.length > 1) this.showAllLob = true;
                else this.filterForm.patchValue({ lob: this.lobInfo[0].lob });
                this.loading = false;
            }
        }, error => {
    ...
    submitFilter(type) {
        ...
        if (type === 1) {
            ...
            dataPos = {
                idp_year: this.filterForm.get('idp_year').value,
                status: this.filterForm.get('status').value,
                pers_no: this.filterForm.get('persno').value,
                rept_to: this.filterForm.get('reptto').value,
                emp_status: this.filterForm.get('empstatus').value,
                lob: this.filterForm.get('lob').value
            }

            this._POST_api_Service.POST_IDP_data(INVars.postSearchidpAPI, dataPos).subscribe(data => {
            ...
            this._POST_api_Service.POST_IDP_data(INVars.downloadAPI, dataPos).subscribe(data1 => {
                this.downloading = false;
                this.applListData = data1;

            }, error => {
                this.downloading = true;
                console.log('[ERROR - Download IDP List] ' + error);
            });

            this._POST_api_Service.POST_IDP_data(INVars.downloadNoIDPAPI, dataPos).subscribe(data1 => {
                this.downloading2 = false;
                this.applListNoData = data1;

            }, error => {
                this.downloading2 = true;
                console.log('[ERROR - Download IDP User w No IDP] ' + error);
            });
    ...
    checkSelectedFunc(val) {
        let checked = false;
        let idx = this.selMobFunctions.indexOf(val);
        if (idx > -1)
            checked = true;
    ...
    idIDPForm;
    openidpDetails(item) {
        this.idIDPForm = item.id;
        this.loading2 = true;
        $('#info_tab').click();
        this.idpDetails = item;

        let posData = { id: item.id };
        ...
        this._POST_api_Service.POST_IDP_data(INVars.getidpDetails, posData).subscribe(res => {
            ...
            this.myIDPStatus = this.idpSummary.details[0].status;
            this.myIDPComplStart = this.idpSummary.idp[0].idp_cmpl_start;
            this.myIDPComplDue = this.idpSummary.idp[0].idp_cmpl_due;
            this.myIDPStart = this.idpSummary.idp[0].idp_start;
            this.myIDPDue = this.idpSummary.idp[0].idp_due;
    ...
    supRevert() {
        ...
        this._POST_api_Service.POST_IDP_data(api, posData).subscribe(res => {
            if (res.status === 'OK') {
                ...
                let y = this.pagedItems.findIndex(a => a.id === this.idIDPForm);
                let x = this.myidp_text_en;


                if (y >= 0) {
                    switch (this.pagedItems[y].text_en) {
                        case 'IDP Completion': this.pagedItems[y].text_en = "IDP Action Plan In Progress";
                            break;
                        case 'IDP Action Plan In Progress': this.pagedItems[y].text_en = "Supervisor Acknowledgement";
                            break;
                        case 'Supervisor Acknowledgement': this.pagedItems[y].text_en = "IDP Setting";
                            break;
                    }
                }

                if (x != '') {
                    switch (this.myidp_text_en) {
                        case 'IDP Completion': this.myidp_text_en = "IDP Action Plan In Progress";
                            break;
                        case 'IDP Action Plan In Progress': this.myidp_text_en = "Supervisor Acknowledgement";
                            break;
                        case 'Supervisor Acknowledgement': this.myidp_text_en = "IDP Setting";
                            break;
                    }
                }
                ...
                $('#closeReverting').click();
                $('#closeReverting2').click();
    ...
    //     // console.log('tengoksini', this.routeStaffId);
    ...
    selectRoute = false;
    staff_no = this.currentUser.userid;
    getSupvforRoute(e) {
        this.selectRoute = true;
        if (e.length > 2) {
            this._POST_api_Service.POST_IDP_data(INVars.postGetSupervisor, { ownStaffNo: this.staff_no, text: e }).subscribe(dataRes => {
                this.getSupvRouteDataResults = dataRes;
            });
        }
    ...
    submitRoute() {
        ...
        this._POST_api_Service.POST_IDP_data(INVars.postRerouteSp, data).subscribe(dataRes => {

            if (dataRes.status === "OK") {

                this.myidp_reptToName = this.nameSVRoute;
                //$('#successRoute').click();
            }
        })
    }
    ...
    download() {
        ...
        a.download = 'IDP_Tracking_' + dateToday + '.csv';
        a.click();
        this.downloading = false;
        return 'success';
    }
    ...
    download2() {
        this.downloading2 = true;
        var csvData = this.ConvertToCSV(this.applListNoData);
        var a = document.createElement("a");
        a.setAttribute('style', 'display:none;');
        document.body.appendChild(a);
        var blob = new Blob([csvData], { type: 'text/csv' });
        ...
        let todayDate = new Date();
        let dateToday = (todayDate.getFullYear() + '' + ((todayDate.getMonth() + 1)) + '' + todayDate.getDate() + '' + todayDate.getHours() + '' + todayDate.getMinutes() + '' + todayDate.getSeconds());
        a.download = 'IDPNotStarted_Tracking_' + dateToday + '.csv';
        a.click();
        this.downloading2 = false;
        return 'success';
    }
