...
import { PTVars } from './career-adv-vars';
...
@Component({
    selector: 'app-career-adv',
    templateUrl: './career-adv.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['../job-css.css']
})

export class CareerAdvComponent implements OnInit, AfterViewInit {
    ...
    detailJobPost = PTVars.detailJobPost;editJobPost = PTVars.editJobPost;
    ...
    public termPosName: string;
    public termCompany: string;
    public termLOB: string;
    public termDepartment: string;
    ...
    getJobPostList = PTVars.getJobPostList;
    ...
    ngAfterViewInit() {
        this._script.loadScripts('app-career-adv',
        [
    ...
    ngOnInit() {
        ...
        this.delCareerAdsForm = new FormGroup({
            delCareerAdsIdx: new FormControl(null, Validators.required),
        });
    ...
    getReportFilter() {
        type jobPost = {
            pos_id: string, pos_name: string, company: string, lob: string, dep: string
        };
        let myarray: jobPost[] = [];
        this._GET_api_Service.GET_data(this.getJobPostList).subscribe(data => {
            for(let i=0; i<data.length; i++){
                myarray.push({
                    pos_id: data[i].Position_ID,
                    pos_name: data[i].Positions,
                    company: data[i].Company,
                    lob: data[i].LOB_Desc,
                    dep: data[i].Org_Unit_Department,
                });
            }
            this.data2 = myarray;
            this.setPage(1);
    ...
    redirect(pos_id: number) {
        this.routers.navigate(['admin/job/advertisement/new-career/detail', pos_id]);
    }
    ...
    delCareerAdsForm: FormGroup;
    act: string;
    dataUpdCareerAds;
    // open model and set value
    delCareerAds(idx) {
        this.delCareerAdsForm.setValue({ delCareerAdsIdx: idx });
    }
    removeJobPost = PTVars.removeJobPost;
    //postDeleteAorAPI = this.apiUrl + this.removeJobPost + this.token;
    onDelCareerAdsFormSubmit(): void {
        this.act = "Delete";
        this.dataUpdCareerAds = {
            positionId: this.delCareerAdsForm.get('delCareerAdsIdx').value
        }
        let careerAdsSend = this._POST_api_Service.POST_data(this.removeJobPost, this.dataUpdCareerAds);
        let ret = careerAdsSend.subscribe(dataAorRes => {
            this.getReportFilter();
            // this.dataUpdCareerAds = dataAorRes;
            // if (this.dataUpdCareerAds.status == "OK") {
