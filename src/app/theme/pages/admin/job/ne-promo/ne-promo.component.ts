...
import { NEPVars } from './ne-promo-vars';
...
@Component({
    selector: 'app-ne-promo',
    templateUrl: '../shared/adv-prof-list/job-adv-prof.component.html',//./job-profile.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['../job-css.css']
})

export class NEPromoComponent {//} implements OnInit {
    posId = GlobalVariable.POS_ID; posName = GlobalVariable.POS_NAME; staffId = GlobalVariable.STAFF_ID;
    title1 = NEPVars.title1; title2 = NEPVars.title2; pageSize = NEPVars.pageSize;
    apiUrl = NEPVars.jobProfileSearchAPI;
    totalRes = '0';
    displayTbl = false; errNoResult = NEPVars.errNoResult; errSearch = NEPVars.errSearch;
    ...
    loading = false;
    mySearch: string; myLob: string; myType = NEPVars.myType;
    isAdv = false;
    ...
    getLOBAPI = NEPVars.getLOBJobAdsAPI;
    optLob: LOB = new LOB();
    optLobList = Array<lobArr>();
    searchByLOB = NEPVars.jobAdvSearchLOB;
    ...
    getProfileData2(term: string, lob: string) {
        ...
        this.routers.navigate([NEPVars.rNEPromoSearch, term, lob]);
    }
    ...
    goToJobDetails(posId) {
        this.routers.navigate([NEPVars.rJobDetail, posId]);
    }
