import { ComponentFactoryResolver, Component, OnInit, AfterViewInit, ViewEncapsulation } from '@angular/core';
import { Http, Response } from '@angular/http';
import "rxjs/add/operator/map";
import { Observable } from "rxjs/Rx";
import { INVars } from './recruitment-vars';
import { GlobalVariable } from "../../../../../../environments/environment";
import { DatePipe ,Location} from '@angular/common';
import { Routes, Router, RouterModule, ActivatedRoute, NavigationStart, ActivatedRouteSnapshot, NavigationEnd, Event as NavigationEvent } from '@angular/router';
import { ScriptLoaderService } from '../../../../../_services/script-loader.service';
import { GET_Service } from '../../../../api/get.service';
import { POST_Service } from '../../../../api/post.service';
import { Headers, RequestOptions } from '@angular/http';
import { AlertService } from '../../../../../auth/_services/alert.service';
import { AlertComponent } from '../../../../../auth/_directives/alert.component';
import { FormGroup, FormControl, Validators, FormBuilder, FormArray } from '@angular/forms';
import { ImageService } from '../../../user/nomination/ImageService';
import * as pdfMake from 'pdfmake/build/pdfmake.js';
import * as pdfFonts from 'pdfmake/build/vfs_fonts.js';
import { NotifierService } from 'angular-notifier';
import * as moment from 'moment';

pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
    selector: 'app-recruitment',
    templateUrl: './recruitment.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['./recruitment.component.css']
})

export class recruitmentComponent implements OnInit, AfterViewInit {
  
  env = GlobalVariable.ENV_NAME;
  env_prod = false;
  
  private readonly notifier: NotifierService;
    constructor(
        private _GET_api_Service: GET_Service, private _POST_api_Service: POST_Service,
        private http: Http, private activeRoute: ActivatedRoute, private routers: Router,
        private datePipe: DatePipe, private _script: ScriptLoaderService, 
        private imageService: ImageService,notifierService: NotifierService,      
        private _alertService: AlertService, private cfr: ComponentFactoryResolver,
        private fb: FormBuilder,private _location: Location) {  
          this.notifier = notifierService;   
          this.ratingForm = this.fb.group({
            ratingF: new FormControl('', [Validators.required,Validators.min(1),Validators.max(30)]),
            ratingNe: new FormControl('', [Validators.required,Validators.min(0),Validators.max(10)]),
            remarkF: new FormControl(''),
            overallRem: new FormControl('', Validators.required),
            panelResult: new FormControl('', Validators.required),
            aux: this.fb.array([]),
            functNE: this.fb.array([]),
          });  
          this.reportForm = this.fb.group({
            rept: this.fb.array([]),
          });                 
    }
    loadingFinal = false;
    loading = false;
    nodata = true;
    displayEmpty = false;
    loadingPanelist = true;
    loadingHead = true;
    loading2 = true;
    loadingSuccess = true;
    loadingAsmt = true;
    loadingResult = true;
    loadingRept = true;
    loadingHistory =  true;
    displayRept = false;
    displayHistory = false;
    clicked = false;
    submitted = false;
    submittedF = false;
    fontstyle = "nav-link a font-lato"; 
    fontstyleH = "nav-link a font-lato"; 
    tabclass = "tab-pane"; 
    ratingForm: FormGroup;
    funcForm: FormGroup;
    ratingGroup: FormArray;
    reportForm: FormGroup;
    reportGroup: FormArray;
    declareForm: FormGroup;
    totalResult;
    totalResultNe;
    totalSuccess = 0;
    totalSuccessNe = 0;
    totalFunc = 0;
    totalFuncNe = 0;
    myDate;
    ischeck: boolean = false;


    ngOnInit() { 
      if(this.env === 'prod')
               this.env_prod = true;
            else
               this.env_prod = false;

      this.myDate = 'Date: '+this.datePipe.transform(new Date(), 'dd-MMM-yyyy'); 
      this.checkLevel();   
      this.getPanelist();
      this.getHeadList();    
      this.getHistoryList();
      this.dataCandTtl;
     
      this.declareForm= new FormGroup({
        chxBoxAgree: new FormControl(null, Validators.required),
       });

      }

    // reset checkbox declare form
    resetdeclareForm() {
      this.declareForm.setValue({
        chxBoxAgree: "",
    });
   }

    ngAfterViewInit() {
    }

    checkLevel() {
      let usrRole = (JSON.parse(localStorage.getItem('currentUser')).job_role);
      if ((!/8/i.test(usrRole))) {
          this.routers.navigate(['/admin/unauthorized']);
          return false;
      }
    }
    
    //start history
    HistoryData: any[];  
    getHistoryList() {
      this.loading2 = true;
      type TrackingData = {          
        id,job_title,location,currentStatus,currStatus,start_time,venue,venue_type,intvwReportDt,
        completed_on,dtdiff,report,image_url
      }; 
      let historyArray:TrackingData[] = []; 
      let imgSrcH;
      this._GET_api_Service.GET_REC_DATA('/recruitment/user/getHistory').subscribe(data => {
        //console.log(data)   
          this.loading2 = false;      
          this.loadingHistory = false;
          for (let i= 0; i < data.length; i++) {

            let imgURL = data[i].job_image_url;
            if(imgURL){
                imgSrcH =  GlobalVariable.BASE_API_URL + '/get/image/' + data[i].job_image_url + "?api_key=" + GlobalVariable.API_KEY;
            }

            historyArray.push({
              id: data[i].id,
              job_title: data[i].job_title,
              location: data[i].location,
              currentStatus: data[i].currentStatus,
              currStatus: data[i].currStatus,
              start_time: data[i].start_time,
              venue: data[i].venue,
              venue_type: data[i].venue_type,
              intvwReportDt: data[i].intvwReportDt,
              completed_on: data[i].last_update_on,
              dtdiff: data[i].dtdiff,
              report: data[i].report,
              image_url: imgSrcH   
            });                        
          }

          this.HistoryData = historyArray;  
         
      }, error => {
          console.log('[ERROR - Fail to get history data] ' + error);
          this.loadingHistory = true;
          this.loading2 = true;
      });
    }

    checkStatus(status, step){
      if(step == 1){
        return true;
      }

      if(step == 2){
        if(status == 'Assessment In Progress' || status == 'Report Submission' || status == 'Completed'){
            return true;
        }
        else
            return false;
      }

      if(step == 3){
        if(status == 'Completed' || status == 'Report Submission'){
            return true;
        }
        else
            return false;
      }

      if(step == 4){
        if(status == 'Completed'){
            return true;
        }
        else
            return false;
      }
    }

    progWidth = '0%';
    chkProgressWidth(status){
        if(status == 'Interview Session') 
            return '0%';
        else if(status == 'Assessment In Progress')
            return '33.3%';
        else if(status == 'Report Submission')
            return '66.6%';
        else
            return '100%';
    }
    //end history

    HeadData;
    showpending;
    getHeadList() {
      let i;
      this.loading2 = true;
      this._GET_api_Service.GET_REC_DATA('/recruitment/user/getModList').subscribe(data => {
        this.loading2 = false;
        //console.log(data)         
          this.loadingHead = false;
          this.HeadData = data;
                   
          if (this.HeadData.length === 1 && this.HeadData[0].cntNotCmplt<1){
            this.showpending = false;
            this.getReport(this.HeadData[0].adv_id);          
          }
          else if(this.HeadData.length === 1 && this.HeadData[0].cntNotCmplt>0){
            this.showpending = true;
          }

      }, error => {
          console.log('[ERROR - Fail to get head complete list] ' + error);
          this.loadingHead = true;
          this.loading2 = true;
      });
    }
    
    isHead; overdue = false;  
    isFirst; nextActiveInt;
    panelistInfo: any[];
    panelData; panelistHistory;historyList;
    sessionList = [];
    getPanelist() {
        this.loading2 = true;
        this.nodata = true;
        
        this._GET_api_Service.GET_REC_DATA('/recruitment/user/getItvSessionbyStaffNo').subscribe(data => {
          this.loading2 = false;
         
          this.loadingPanelist = false;
           
            this.sessionList = data.dataSession;
            this.historyList = data.dataHistory;

            if(this.sessionList.length>0)
              this.nodata = false;
            else
              this.displayEmpty = true;

            this.isFirst = this.sessionList.findIndex(t=>t.sequence == '1');
            this.nextActiveInt = this.isFirst == '-1' ? null :this.sessionList[this.isFirst].start_time;
            
            this.isHead = this.sessionList.filter(t=>t.name.trim() == 'Head Interview Panel');
          
          if(this.historyList[0].cntHistory>0 && this.nodata == true){ 
            this.displayHistory = true;
            this.tabclass = "tab-pane active"
            this.fontstyleH = "nav-link a font-lato blue-text active";
          }
  
          }, error => {
              console.log('[ERROR - Fail to get applicant list by staffno] ' + error);
            
              this.loadingPanelist = true;
              this.loading2 = true;
        }); 
        
        
    }

    returndate(mydate){
      return this.datePipe.transform(mydate, 'dd-MMM-yyyy');       
    }

    // download user career profile
    cpSummary; cpExperience; cpEducation; cpProfile; cpPrevWork; cpAward; cpProfCert; cpProfileName; 
    cpProfileAge; cpProfilePosition; cpProfileStaff_No; cpProfilePost_Desc; cpProfileimage_url;
    cpProfilePersNo; cpProfileRept_To_Name; cpProfileUnit; cpProfileBirth_Date; cpPrevWorkEmployer;
    userCP: any; titleCPPdf: String; docCPDefinition; theDate; userId; imgDataUrl; 

    downloading3 = true;

    getUserCareerProfile(itemCP){
        this.userCP = itemCP;
        let dataCP = {
          staffNo : itemCP,
        }
       
        this._POST_api_Service.POST_IDP_data('/tc/career_profile', dataCP).subscribe(res => {
          this.cpSummary= res.career;
          this.cpProfile = this.cpSummary.profile;
          this.cpProfileName = this.cpProfile[0].Name;
          this.cpProfileAge = this.cpProfile[0].Age;
          this.cpProfilePosition = this.cpProfile[0].Post_Desc;
          this.cpProfileStaff_No = this.cpProfile[0].Staff_No;
          this.cpProfileimage_url = this.cpProfile[0].image_url;
          this.cpProfilePersNo = this.cpProfile[0].Pers_no;
          this.cpProfileRept_To_Name = this.cpProfile[0].Rept_To_Name;
          this.cpProfileUnit = this.cpProfile[0].Sub_Org_Unit_Desc;
          this.cpProfileBirth_Date = this.cpProfile[0].Birth_Date;
          this.cpEducation = this.cpSummary.education;
          this.cpPrevWork = this.cpSummary.previous;
          this.cpExperience = this.cpSummary.exprience;
          this.cpAward = this.cpSummary.award;
          this.cpProfCert = this.cpSummary.profCert;
        
          let theUrl = `${INVars.APIGetImg}/${this.cpProfileimage_url}`;
          this.theDate = this.datePipe.transform(new Date(), 'dd-MM-yyyy h:mm a');
          let profilePictureSend = this._GET_api_Service.GET_Picture(theUrl);
          try {
            this.userId = JSON.parse(localStorage.getItem('currentUser')).userid;
          } catch (e) {
            console.error("Failed to get localStorage for currentUser");
          }
          profilePictureSend.subscribe(pictureResults => {
            this.imgDataUrl = '';
            let profilePictureBase64 = this._GET_api_Service.GET_Base64(pictureResults);
            profilePictureBase64.subscribe(myData => {
              this.imgDataUrl = myData;
            });
          }, () => {
            this.imgDataUrl = 'data:image/jpg;base64,';
          });
          this.titleCPPdf = `CAREER PROFILE_${this.cpProfile[0].Staff_No}.pdf`;
          setTimeout(() => {
            this.downloading3 = false;
            let profile_img = this.imgDataUrl;
            let myCP_Name = this.cpProfileName;
            let myCP_reptToName = this.cpProfileRept_To_Name === null || this.cpProfileRept_To_Name === '' ? '-' : this.cpProfileRept_To_Name;
            let myCP_pers_no = this.cpProfilePersNo;
            let myCP_staff_no = this.cpProfileStaff_No;
            let myCP_birthdate = this.cpProfileBirth_Date;
            let myCP_position = this.cpProfilePosition;
            let myCP_age = this.cpProfileAge;
    
            this.docCPDefinition = {
              pageSize: 'A4',
              pageMargins: [20, 90],
              watermark: { text: `By: ${this.userId}@${this.theDate}`, color: '#e0e0d1', opacity: 0.3, bold: true },
              background: function(page) {
                if (page !== 1) {
                  return [
                    {
                      columns: [
                        {
                          width: 175,
                          alignment: 'center',
                          table: {
                            width: ['auto'],
                            body: [
                              [{ image: profile_img, width: 95, height: 95 }],
                              [{ text: `\n` }],
                              [{ text: `${myCP_Name}`, style: 'profile_name' }],
                              [{ columns: [{ text: `Personnel No `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_pers_no} `, style: 'blackSize10' },] }],
                              [{ columns: [{ text: `Staff ID `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_staff_no} `, style: 'blackSize10' },] }],
                              [{ columns: [{ text: `Position  `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_position} `, style: 'blackSize10' },] }],
                              [{ columns: [{ text: `Reporting To `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_reptToName} `, style: 'blackSize10' },] }],
                              [{ columns: [{ text: `Birth Date `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_birthdate} `, style: 'blackSize10' },] }],
                              [{ columns: [{ text: `Age `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_age} `, style: 'blackSize10' },] }],
                            ]
                          },
                          layout: 'noBorders',
                          margin: [20, 105, 0, 0]
                        }
                      ]
                    }
                  ]
                }
              },
              header: {},
              footer: function(currentPage, pageCount) {
                return {
                  text: 'Page : ' + currentPage.toString() + ' / ' + pageCount,
                  color: 'gray',
                  bold: 'true',
                  alignment: 'right',
                  fontSize: 11,
                  margin: 20
                }
              },
              content: [],
              images: {
                logoEra: 'data:image/png;base64,',
                logoTM: 'data:image/png;base64,'
              },
              styles: {
                era_title: {
                  color: '#ea1819',
                  fontSize: 20,
                  bold: true,
                  alignment: 'center',
                  margin: [0, 20, 0, 0]
                },
                profile_name: {
                  bold: true,
                  color: '#fd5806',
                  fontSize: 12,
                  alignment: 'left',
                },
                header: {
                  color: '#fd5806',
                  bold: true,
                  fontSize: 14,
                },
                profile_header: {
                  color: '#5e6063',
                  bold: true,
                  fontSize: 12,
                  alignment: 'left',
                },
                wm_title: {
                  bold: true,
                  fontSize: 30,
                  color: '#5e6063',
                  opacity: 0.3
                },
                wm_staffId: {
                  bold: true,
                  fontSize: 40,
                  color: '#5e6063',
                  opacity: 0.3
                },
                lst_data: {
                  bold: true,
                  fontSize: 11,
                },
                lst_title2: {
                  fontSize: 11,
                  bold: false,
                  color: '#5c6066',
                },
                lst_data2: {
                  bold: true,
                  fontSize: 11,
                  margin: [20, 0, 0, 5]
                },
                lst_data_color: {
                  bold: true,
                  fontSize: 11,
                  color: '#fd5806'
                },   
                red10: {
                  fontSize: 10,
                  color: 'red',
                  bold: true
                },
                lst_title_no: {
                  fontSize: 11,
                  bold: false,
                  color: '#5c6066',
                },
                lst_title: {
                  fontSize: 11,
                  bold: false,
                  color: '#5c6066',
                  margin: [20, 0, 0, 0]
                  },
                  bold: {
                    bold: true
                  },
                  greySize10: {
                    fontSize: 10,
                    color: '#5e6063',
                    bold: true,
                    alignment: 'left'
                  },
                  blackSize10: {
                    fontSize: 10,
                    bold: true,
                    alignment: 'left'
                  }
                }
              }
    
              // Populate the header of the PDF 
              this.docCPDefinition.header = {
                table: {
                  widths: ['auto', '*', 'auto'],
                  //headerRows: 1,
                  body: [
                    [{ rowSpan: 3, image: 'logoEra', fit: [80, 80], margin: [0, 7, 0, 10], bold: true }, { rowSpan: 3, text: `EMPLOYEE CAREER PROFILE`, style: 'era_title' }, { rowSpan: 3, image: 'logoTM', fit: [100, 100] }],
                    [{ text: `` }, '', ''],
                    [{ text: ``, margin: [0, 0, 0, 10], bold: true }, '', ''],
                    [{ colSpan: 3, text: '', fillColor: '#ff3300' }]
                  ]
                },
                layout: 'noBorders',
                margin: [20, 20, 20, 40]
              };
    
              // Populate the content of PDF
              let myCPContent;
              this.docCPDefinition.content = [];
    
              // We invoke another request to convert the blob to Base64
              myCPContent = [
                { // 0.
                  table: {
                    widths: [350],
                    body: [
                      [{ text: '' }]
                    ]
                  },
                  layout: 'noBorders',
                  margin: [0, 20, 0, 5]
                },
                { // 1.
                  columns: [
                    {
                      width: 165,
                      alignment: 'center',
                      table: {
                        width: ['auto'],
                        body: [
                          [{ image: profile_img, width: 95, height: 95 }],
                          [{ text: `\n` }],
                          [{ text: `${myCP_Name}`, style: 'profile_name' }],
                          [{ columns: [{ text: `Personnel No `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_pers_no} `, style: 'blackSize10' },] }],
                          [{ columns: [{ text: `Staff ID `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_staff_no} `, style: 'blackSize10' },] }],
                          [{ columns: [{ text: `Position  `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_position} `, style: 'blackSize10' },] }],
                          [{ columns: [{ text: `Reporting To `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_reptToName} `, style: 'blackSize10' },] }],
                          [{ columns: [{ text: `Birth Date `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_birthdate} `, style: 'blackSize10' },] }],
                          [{ columns: [{ text: `Age `, style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${myCP_age} `, style: 'blackSize10' },] }],           
                        ]
                      },
                      layout: 'noBorders',
                    },
                    {
                      width: 'auto', margin: 5,
                      type: 'none',
                      ul: [
                        {   //ul[0]
                          table: {
                            body: [
                              [{ text: 'JOB EXPERIENCE', style: 'header' }]
                            ]
                          }, 
                          layout: 'noBorders',
                        },
                        {   //ul[1]
                          text: '\n'
                        },
                        {   //ul[2]
                          type: 'none',
                          ul: [
                            { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                          ]
                        },
                        {   //ul[3]
                          text: '\n\n'
                        },
                        {   //ul[4]
                          table: {
                            body: [
                              [{ text: 'EDUCATION BACKGROUND', style: 'header' }]
                            ]
                          }, 
                          layout: 'noBorders',
                        },
                        {   //ul[5]
                          text: '\n'
                        },
                        {   //ul[6]
                          type: 'none',
                          ul: [
                            { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                          ]
                        },
                        {   //ul[7]
                          text: '\n\n'
                        },
                        {   //ul[8]
                          table: {
                            body: [
                              [{ text: 'PROFESSIONAL QUALIFICATION', style: 'header' }]
                            ]
                          }, layout: 'noBorders',
                        },
                        {   //ul[9]
                          text: '\n'
                        },
                        {   //ul[10]
                          type: 'none',
                          ul: [
                            { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                          ]
                        },
                        {   //ul[11]
                          text: '\n\n'
                        },
                        {   //ul[12]
                          table: {
                            body: [
                              [{ text: 'PREVIOUS WORKING EXPERIENCE', style: 'header' }]
                            ]
                          }, layout: 'noBorders',
                        },
                        {   //ul[13]
                          text: '\n'
                        },
                        {   //ul[14]
                          type: 'none',
                          ul: [
                            { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                          ]
                        },
                        {   //ul[15]
                          text: '\n\n'
                        },
                        {   //ul[16]
                          table: {
                            body: [
                              [{ text: 'AWARDS / MERIT', style: 'header' }]
                            ]
                          }, layout: 'noBorders',
                        },
                        {   //ul[17]
                          text: '\n'
                        },
                        {   //ul[18]
                          type: 'none',
                          ul: [
                            { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                          ]
                        },
                        {   //ul[19]
                          text: '\n\n'
                        }
                      ]
                    }
                  ]
                }
              ];
               // Populate - JOB EXPERIENCE (ul[2])
              if (this.cpExperience.length) {
                let cpExperienceList = {
                  type: 'none',
                  ul: []
                };
                let num = 1;
                this.cpExperience.forEach(function (myVal) {
                  
                  let dp:DatePipe = new DatePipe('en-US');
                  let dateJoinStr = dp.transform(new Date(), 'dd-MM-yyyy');
          
                  let myRow = [];
                  var n = (num < 10) ? ('0' + num++) : num++;
                  myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Position ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Post_Desc}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Company ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Org_Unit_Desc}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Date Join ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Date End ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.End_Date}`, style: 'lst_data' }] });
                 
                  myRow.push({ text: '\n' });
                  cpExperienceList.ul.push(myRow);
                });
                myCPContent[1].columns[1].ul[2] = cpExperienceList;
              }
    
              // Populate - EDUCATION BACKGROUND (ul[6])
              if (this.cpEducation.length) {
                let eduBackList = {
                  type: 'none',
                  ul: [ ]
                };
                let num = 1;
                this.cpEducation.forEach(function (myVal) {
                  var n = (num < 10) ? ('0' + num++) : num++;
                  let myRow = [];
                  myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Institution ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.University}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Educational level ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Qualification_Desc}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Branch of Study ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Branch_of_Study}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Certificate ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Certificate}`, style: 'lst_data' }] });
                  myRow.push({ text: '\n' });
    
                  eduBackList.ul.push(myRow);
                });
                myCPContent[1].columns[1].ul[6] = eduBackList;
              }  
    
              //Populate - Prof Qualification (ul[10])
              if (this.cpProfCert.length) {
                let cpProfCertList = {
                  type: 'none',
                  ul: [ ]
                };
                let num = 1;
                this.cpProfCert.forEach(function (myVal) {
                  var n = (num < 10) ? ('0' + num++) : num++;
                  let myRow = [];
                  myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Cert Name ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Qualification}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Cert Issuer ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date}`, style: 'lst_data' }] });
                  // myRow.push({ columns: [{ width: 100, text: 'Cert Year ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.certYear}`, style: 'lst_data' }] });
                  myRow.push({ text: '\n' });
                  cpProfCertList.ul.push(myRow);
                });
                myCPContent[1].columns[1].ul[10] = cpProfCertList;
              }
    
              // Populate - Previous EXPERIENCE (ul[2])
              if (this.cpPrevWork.length) {
                let cpPrevWorkList = {
                  type: 'none',
                  ul: [ ]
                };
                let num = 1;
                this.cpPrevWork.forEach(function (myVal) {
                  
                  let dp:DatePipe = new DatePipe('en-US');
                  let dateJoinStr = dp.transform(new Date(), 'dd-MM-yyyy');
          
                  let myRow = [];
                  var n = (num < 10) ? ('0' + num++) : num++;
                  myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Position ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Position}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Company ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Employer}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Date Join ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date}`, style: 'lst_data' }] });
                  myRow.push({ columns: [{ width: 100, text: 'Date End ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.End_Date}`, style: 'lst_data' }] });
                  
                  myRow.push({ text: '\n' });
                  cpPrevWorkList.ul.push(myRow);
                });
    
                myCPContent[1].columns[1].ul[14] = cpPrevWorkList;
              }
    
              //Populate - Awards/Merit (ul[16])
              if (this.cpAward.length) {
                let cpAwardList = {
                  type: 'none',
                  ul: [ ]
              };
              let num = 1;
              this.cpAward.forEach(function (myVal) {
                var n = (num < 10) ? ('0' + num++) : num++;
                let myRow = [];
                myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Award ' }], style: 'lst_title_no' }, { width: 5, text: ':', style: 'lst_data' }, { text: `${myVal.Award}`, style: 'lst_data' }] });
                myRow.push({ columns: [{ width: 100, text: 'Awarded On ', style: 'lst_title' }, { width: 5, text: ':', style: 'lst_data' }, { text: `${myVal.End_Date} / ${myVal.Start_Date}`, style: 'lst_data' }] });
                myRow.push({ text: '\n' });
                cpAwardList.ul.push(myRow);             
              });
    
              myCPContent[1].columns[1].ul[18] = cpAwardList;
             }  
            this.docCPDefinition.content.push(myCPContent);
            this.downloadCPForm();
          }, 1500)
        }, error => {
          //this.loading2 = true;
          console.log('[ERROR] Fail to fetch Career Profile details: ' + error);
        });
    }

    downloadCPForm() {
        pdfMake.createPdf(this.docCPDefinition).download(this.titleCPPdf);
        this.imgDataUrl = '';  
    }

    returnImg(id){
      let imgSrc;
      let imgURL = id;
      if(imgURL){
        imgSrc =  GlobalVariable.BASE_API_URL + '/get/image/' + id + "?api_key=" + GlobalVariable.API_KEY;
      }
      else
        imgSrc = './assets/app/media/img/users/ghcm-user-default.jpg';
      return imgSrc
    }

    //Add info for declaration form
    selPosTitle; selPosNo; selDeclare; selPosition
    selPosId; selAdsId; selNo; selVenueType;
    selVenue; selStart; selEnd; selPosPnlId;
     
    expandCompareRow(fields){
      fields.expanded = !fields.expanded;
      this.selAdsId = fields.adv_id;
      this.selPosition = fields.position;
      this.selPosTitle = fields.panelPosition;
      this.selPosPnlId = fields.panelPositionID;
      this.selPosId = fields.position_id;
      this.selVenue = fields.venue;
      this.selVenueType = fields.venue_type;
      this.selStart = fields.start_time;
      this.selEnd = fields.end_time;
      this.selNo = fields.no;
      this.selDeclare = fields.panelDeclaration;
    
    }

   
    successData; personal; functional; rating = [];
    getAsmtRating(id,no) {
      let dataCP = {
        appID : id,
        rowno : no,
      }
  
      //console.log(dataCP)
      this.loadingAsmt = true;
      this.totalSuccess = 0;
      this.totalFunc = 0;
      //this.totalResult = 0;
      this._POST_api_Service.POST_IDP_data('/recruitment/user/getRatingByAppl', dataCP).subscribe(data => {
               
          this.loadingAsmt = false;
          this.successData = data.summary;
          this.personal = this.successData.personal;
          this.rating = this.successData.rating.filter(t=>t.qType == 'SUCCESS');
         
          this.functional = this.successData.rating.filter(t=>t.qType == 'Functional');
         
          this.totalResult = this.personal[0].finalresult === '' ||  this.personal[0].finalresult === null ? 0 : this.functional[0].rating+this.personal[0].totalRating;
        
          this.clicked = true;
          this.fontstyle = "navv2-link a font-lato";
          let theUrl = `${INVars.APIGetImg}/${this.personal[0].image_url}`;
          let profilePictureSend = this._GET_api_Service.GET_Picture(theUrl);
          profilePictureSend.subscribe(pictureResults => {
            this.imgDataUrl = '';
            let profilePictureBase64 = this._GET_api_Service.GET_Base64(pictureResults);
            profilePictureBase64.subscribe(myData => {
                this.imgDataUrl = myData;
            });
         
          }, () => {
              this.imgDataUrl = 'data:image/jpg;base64,';
          });

          const arryOfratingGroup = this.rating.map(item => {
            return this.createRatingGroup(item.ratingID, item.rating, item.remarks,item.SUC_DESC,item.SUC_NAME,item.DETAILS);
          });
          this.ratingForm = this.fb.group({
            ratingF: [this.functional[0].rating,[Validators.required,Validators.min(1),Validators.max(30)]],
            remarkF: [this.functional[0].remarks],
            overallRem: [this.personal[0].finalremarks,[Validators.required]],
            panelResult: [this.personal[0].finalresult === '' ||  this.personal[0].finalresult === null ? '' : this.personal[0].finalresult,[Validators.required]],
            aux: this.fb.array(arryOfratingGroup)
          });

      }, error => {
          console.log('[ERROR - Fail to get rating result for app id '+id+' ] ' + error);
          this.loadingAsmt = true;
      });
      
    }

   //Post Assessment Rating for NE Promo
   functionalNe = [];
   postAssRatingNE(id,no) {
    let dataAssNe = {
      appID : id,
      rowno : no,
    }

 
    this.loadingAsmt = true;
    this.totalSuccess = 0;
    this.totalSuccessNe = 0;
    this.totalFunc = 0;
    this.totalFuncNe = 0;
   
    this._POST_api_Service.POST_IDP_data('/recruitment/user/getRatingByApplNE', dataAssNe).subscribe(data => {
            
        this.loadingAsmt = false;
        this.successData = data.summary;
        this.personal = this.successData.personal;
        this.rating = this.successData.rating.filter(t=>t.qType == 'SUCCESS');
       
        this.functional = this.successData.rating.filter(t=>t.qType == 'Functional');
      
        this.totalResultNe = this.personal[0].finalresult === '' ||  this.personal[0].finalresult === null ? 0 : this.functional[0].rating+this.functional[1].rating+this.personal[0].totalRating;
     
        this.totalFuncNe = this.functional[0].rating+this.functional[1].rating;
      
        this.clicked = true;
        this.fontstyle = "navv2-link a font-lato";
        let theUrl = `${INVars.APIGetImg}/${this.personal[0].image_url}`;
        let profilePictureSend = this._GET_api_Service.GET_Picture(theUrl);
        profilePictureSend.subscribe(pictureResults => {
          this.imgDataUrl = '';
          let profilePictureBase64 = this._GET_api_Service.GET_Base64(pictureResults);
          profilePictureBase64.subscribe(myData => {
              this.imgDataUrl = myData;
          });
       
        }, () => {
            this.imgDataUrl = 'data:image/jpg;base64,';
        });

        const arryOfratingGroup = this.rating.map(item => {
          return this.createRatingGroup(item.ratingID, item.rating, item.remarks,item.SUC_DESC,item.SUC_NAME,item.DETAILS);
        });

        const arryOfFunctionalGroup = this.functional.map(item => {
          return this.createFuncGroupNE(item.ratingID, item.rating, item.remarks,item.fSUC_DESC,item.fDETAILS,item.fSUC_NAME);
        }); 

        this.ratingForm = this.fb.group({
          // ratingF: [this.functional[0].rating,[Validators.required,Validators.min(1),Validators.max(30)]],
          // ratingNe: [this.functional[0].rating,[Validators.required,Validators.min(1),Validators.max(10)]],
          remarkF: [this.functional[0].remarks],
          overallRem: [this.personal[0].finalremarks,[Validators.required]],
          panelResult: [this.personal[0].finalresult === '' ||  this.personal[0].finalresult === null ? '' : this.personal[0].finalresult,[Validators.required]],
          aux: this.fb.array(arryOfratingGroup),
          functNE: this.fb.array(arryOfFunctionalGroup)
        }); 
     }, error => {
        console.log('[ERROR - Fail to get rating result for app id '+id+' ] ' + error);
        this.loadingAsmt = true;
    });
    
  }


    reset(){
      this.clicked = false;
      this.submitted = false;
      this.submittedF = false;
      this.edited = false;
      this.loading = false;
      this.loadingFinal = false;
      this.displayRept = false;
      this.displayEmpty = false;
      this.fontstyle = "nav-link a font-lato"; 
    }

    reset2(){
      this.clicked = true;
    }

    get auxArray() {
      return this.ratingForm.get("aux")["controls"] as FormArray;
    }

    //NE Promo functional Array
    get funcNeArray() {
      return this.ratingForm.get("functNE")["controls"] as FormArray;
    }

    createRatingGroup(ratingID, rating, remark,SUC_DESC,SUC_NAME,DETAILS) {
      return this.fb.group({
        ratingID: [ratingID],
        rating: [rating,[Validators.required,Validators.min(0),Validators.max(10)]],
        remark: [remark],
        SUC_DESC: [SUC_DESC],
        SUC_NAME: [SUC_NAME],
        DETAILS: [DETAILS]
      });
    }

    createFuncGroupNE(ratingID, rating, remarks, fSUC_DESC, fDETAILS, fSUC_NAME) {
      return this.fb.group({
        ratingID: [ratingID],
        rating: [rating,[Validators.required,Validators.min(0),Validators.max(5)]],
        remarks: [remarks],
        fSUC_DESC:[fSUC_DESC],
        fDETAILS:[fDETAILS],
        fSUC_NAME:[fSUC_NAME],
      });
    }

    get reportArray() {
      return this.reportForm.get("rept")["controls"] as FormArray;
    }

    //Change min to 0 -17/9/2021
    createReportGroup(candidateID, score, result, remark) {
      return this.fb.group({
        candidateID: [candidateID],
        score: [score,[Validators.required,Validators.min(0),Validators.max(100)]],
        result: [result,[Validators.required]],
        remark: [remark,[Validators.required]],
      });
    }

    finalScore;gotnextCand;
    submit() {    
      this.submitted = true;
      let i;let mysum = 0; 
      let j = this.ratingForm.get("aux")["controls"] as FormArray;
          
      if (this.ratingForm.valid){
        this.loading = true;
        // save success
        for (i = 0; i < j.length; i++) {
          mysum = mysum + this.ratingForm.value.aux[i].rating;
          let data = {                  
            ratingID: this.ratingForm.value.aux[i].ratingID,
            rating: this.ratingForm.value.aux[i].rating,
            remarks: this.ratingForm.value.aux[i].remark
          }
          let addCr = this._POST_api_Service.POST_REC_data('/recruitment/user/submitRating', data);
          let dataJUDel: any = {};
          addCr.subscribe(dataRes => {
            dataJUDel = dataRes;  
                                                                                         
          }, 
          error => {
              console.log('[ERROR - Submit Rating for SUCCESS]')
          }
        )         
        }  
        //end save success

         //save functional
      
         let data2 = {                  
            ratingID: this.functional[0].ratingID,
            rating: this.ratingForm.get('ratingF').value,
            remarks: this.ratingForm.get('remarkF').value
          }
              
          let addCr2 = this._POST_api_Service.POST_REC_data('/recruitment/user/submitRating', data2);
          let dataJUDel2: any = {};
          addCr2.subscribe(dataRes2 => {
            dataJUDel2 = dataRes2;  
                                                                                                    
          }, 
          error => {
              console.log('[ERROR - Submit Rating for Functional]')
          }
          ) 
                     
         //save result
          if (this.personal[0].tm_band == '1' && (this.personal[0].criteria == '1' || this.personal[0].criteria == '2')){
            //console.log('b1')  
            this.finalScore = (this.personal[0].harrisonResults/100*30) +
            (this.personal[0].functionalResults/100*30) +
            (this.personal[0].TCResults/10*10)
            //{{(item.harrisonResults/100*30)+(item.functionalResults/100*30)+(item.TCResults/20*10) | number : '1.2-2'}}
          }
          else if (this.personal[0].tm_band == '2' && (this.personal[0].criteria == '1' || this.personal[0].criteria == '2')){
            //console.log('b2')
            this.finalScore = (this.personal[0].harrisonResults/100*40) +
            (this.personal[0].functionalResults/100*20) +
            (this.personal[0].TCResults/10*10)
            //{{(item.harrisonResults/100*40)+(item.functionalResults/100*20)+(item.TCResults/20*10) | number : '1.2-2'}}
          }
          else if (this.personal[0].tm_band == '3' && (this.personal[0].criteria == '1' || this.personal[0].criteria == '2')){
            //console.log('b3')
            this.finalScore = (this.personal[0].harrisonResults/100*50) +
            (this.personal[0].functionalResults/100*10) +
            (this.personal[0].TCResults/10*10)
            //{{(item.harrisonResults/100*50)+(item.functionalResults/100*10)+(item.TCResults/20*10) | number : '1.2-2'}}
          }
          else{
            //console.log('unknown')
            this.finalScore = 0;
          }
          
                
          let data3 = {                  
            appID: this.personal[0].id,
            remarks: this.ratingForm.get('overallRem').value,
            result: this.ratingForm.get('panelResult').value,
            totalRating: mysum,
            funcRating: this.ratingForm.get('ratingF').value,
            finalCompScore: this.finalScore.toFixed(2)
          }
         
          let addCr3 = this._POST_api_Service.POST_REC_data('/recruitment/user/updateResult', data3);
          let dataJUDel3: any = {};
          addCr3.subscribe(dataRes3 => {
            dataJUDel3 = dataRes3;     
                         
            this.notifier.notify('success','Candidate has been successfully updated');                                                                        
          }, 
          error => {
            this.notifier.notify('error','Fail to update candidate');
              console.log('[ERROR - Submit Final Result]')
          }
          )
         
          //new 19012021
          this.loading = false;
          this.submitted = false;
          this.edited = false;
          
          this.gotnextCand = this.personal[0].nextCandidate === null || this.personal[0].nextCandidate === '' ? false : true;
          
          if (this.gotnextCand == true){
            //console.log('gotnextcand')
            window.scrollTo({top: 0, behavior: 'smooth'});
            let advid = this.sessionList.findIndex(t=>t.adv_id == this.personal[0].adv_id);
            let rowid = this.sessionList[advid].applicant.findIndex(t=>t.id == this.personal[0].nextCandidate);
            let rowno = this.sessionList[advid].applicant[rowid].no;
          
          //console.log(this.personal[0].nextCandidate+' - '+rowno)
            this.getAsmtRating(this.personal[0].nextCandidate,rowno); //auto next candidate
          }
          else{          
          console.log('donthvnextcand')
            this.reset();
            this.getPanelist();
            this.getHeadList();      
            this.getHistoryList();    
          }          
          //new 19012021


          // .add(() => {
          //   //Called when operation is complete (both success and error)
          //   this.reset();
          //   this.getPanelist();
          //   this.getHeadList();               
          //   //window.scrollTo({top: 0, behavior: 'smooth'});
          // }); 
          //end save result          
      } 
      else{
        this.scrollToError();
      }
    }

    //NE Promo - submit Assessment score
    submitNe() {    

      this.submitted = true;
      let i;let mysum = 0; let mysumNe = 0;
      let j = this.ratingForm.get("aux")["controls"] as FormArray;
          
      if (this.ratingForm.valid){
        this.loading = true;
        // save success
        for (i = 0; i < j.length; i++) {
          mysum = mysum + this.ratingForm.value.aux[i].rating;
          let data = {                  
            ratingID: this.ratingForm.value.aux[i].ratingID,
            rating: this.ratingForm.value.aux[i].rating,
            remarks: this.ratingForm.value.aux[i].remark
          }
          mysumNe = mysum * 3
          
          let addCr = this._POST_api_Service.POST_REC_data('/recruitment/user/submitRating', data);
          let dataJUDel: any = {};
          addCr.subscribe(dataRes => {
            dataJUDel = dataRes;  
                             
          }, 
           error => {
              console.log('[ERROR - Submit Rating for SUCCESS]')
          }
        )         
        }  
        //end save success

         //save functional
        let totalFuncNe = 0;
        
        let k = this.ratingForm.get("functNE")["controls"] as FormArray;
         for (i = 0; i < k.length; i++) {
              let dataNe = {                  
                ratingID: this.ratingForm.value.functNE[i].ratingID,
                rating: this.ratingForm.value.functNE[i].rating,
                remarks: this.ratingForm.value.functNE[i].remarks
              }
              totalFuncNe = totalFuncNe + this.ratingForm.value.functNE[i].rating;
            
          let addCr2 = this._POST_api_Service.POST_REC_data('/recruitment/user/submitRating', dataNe);
          let dataNeFunctional: any = {};
          addCr2.subscribe(dataResNe => {
            dataNeFunctional = dataResNe;  
           
          }, 
          error => {
              console.log('[ERROR - Submit Rating for Functional]')
          }
          ) 
        }   
    
          let data3 = {                  
            appID: this.personal[0].id,
            remarks: this.ratingForm.get('overallRem').value,
            result: this.ratingForm.get('panelResult').value,
            totalRating: mysumNe,
            funcRating: totalFuncNe,
            finalCompScore: 0
          }
         
          let addCr3 = this._POST_api_Service.POST_REC_data('/recruitment/user/updateResult', data3);
          let dataJUDel3: any = {};
          addCr3.subscribe(dataRes3 => {
            dataJUDel3 = dataRes3;     
                          
            this.notifier.notify('success','Candidate has been successfully updated');                                                                        
          }, 
          error => {
            this.notifier.notify('error','Fail to update candidate');
              console.log('[ERROR - Submit Final Result]')
          }
          )
          this.loading = false;
          this.submitted = false;
          this.edited = false;
          
          this.gotnextCand = this.personal[0].nextCandidate === null || this.personal[0].nextCandidate === '' ? false : true;
          
          if (this.gotnextCand == true){
            //console.log('gotnextcand')
            window.scrollTo({top: 0, behavior: 'smooth'});
            let advid = this.sessionList.findIndex(t=>t.adv_id == this.personal[0].adv_id);
            let rowid = this.sessionList[advid].applicant.findIndex(t=>t.id == this.personal[0].nextCandidate);
            let rowno = this.sessionList[advid].applicant[rowid].no;
          
          //console.log(this.personal[0].nextCandidate+' - '+rowno)
            this.postAssRatingNE(this.personal[0].nextCandidate,rowno); //auto next candidate
          }
          else{          
            //console.log('donthvnextcand')
            this.reset();
            this.getPanelist();
            this.getHeadList();      
            this.getHistoryList();    
          }          
      }
      else{
        this.scrollToError();
      }
    }
    // End NE Promo - submit Assessment score


    backClicked() {
      this._location.back();
    }

    edited = false;
    sum(){
      this.edited = true;
      let i;
      let mysum = 0;
           
      let j = this.ratingForm.get("aux")["controls"] as FormArray;

      for (i = 0; i < j.length; i++) {
        mysum = mysum + this.ratingForm.value.aux[i].rating;
      }
  
      this.totalSuccess = mysum;
      this.totalFunc = this.ratingForm.get('ratingF').value === '' ||  this.ratingForm.get('ratingF').value === null ? 0 : this.ratingForm.get('ratingF').value; 
      this.totalResult = this.totalSuccess + this.totalFunc;  
    }

    //NE Promo - calculate total result
    sumNe(){
      this.edited = true;
      let i;
      let mysum = 0;
      let totalFuncNe = 0;
      
      let j = this.ratingForm.get("aux")["controls"] as FormArray;

      for (i = 0; i < j.length; i++) {
        mysum = mysum + this.ratingForm.value.aux[i].rating;
      }

      let k = this.ratingForm.get("functNE")["controls"] as FormArray;

      for (i = 0; i < k.length; i++) {
        totalFuncNe = totalFuncNe + this.ratingForm.value.functNE[i].rating;
      }

          
      this.totalSuccessNe = mysum * 3;
      this.totalFuncNe = totalFuncNe;
      this.totalResultNe = this.totalSuccessNe + this.totalFuncNe;
    
    }



    scrollTo(el: Element): void {
      if (el) {
         el.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }

    scrollToError(): void {
      const firstElementWithError = document.querySelector('.ng-invalid[formControlName]');
      this.scrollTo(firstElementWithError);
    }

    reportData; dataSession;
    dataPanel : any[]; 
    dataCandidate : any[];
    dataCandTtl;
    
    getReport(id) {
      this.loadingRept = true;
      this.displayRept = true;   
      this._GET_api_Service.GET_REC_DATA('/recruitment/user/getReport/'+id).subscribe(data => {
                
          this.loadingRept = false;            
          this.reportData = data.summary;
         
          this.dataSession = this.reportData.datasession;
          this.dataPanel = this.reportData.datapanel;
        
          this.dataCandidate = this.reportData.datacandidate;
          this.dataCandTtl = this.reportData.datacandidate[0].ttlmarks;
         
          const arryOfreportGroup = this.dataCandidate.map(item => {
            return this.createReportGroup(item.candidateID, item.ttlmarks,  item.headresult == null ? item.headresultRpl : item.headresult, item.headremarks == null ? item.headremarksRpl : item.headremarks);
          });
          this.reportForm = this.fb.group({
            rept: this.fb.array(arryOfreportGroup)
          });
      }, error => {
          console.log('[ERROR - Fail to get report for adv id '+id+' ] ' + error);
          this.loadingRept = true;
      });      
    }
    
    appResultData;imgApplicant;
    getApplicantResult(id) {
      this.loadingResult = true;
      this._GET_api_Service.GET_REC_DATA('/recruitment/user/getStaffRept/'+id).subscribe(data => {
                
        this.loadingResult = false;
        this.appResultData = data;     
        let theUrl = `${INVars.APIGetImg}/${this.appResultData[0].image_url}`;
        let profilePictureSend = this._GET_api_Service.GET_Picture(theUrl);
          profilePictureSend.subscribe(pictureResults => {
            this.imgApplicant = '';
            let profilePictureBase64 = this._GET_api_Service.GET_Base64(pictureResults);
            profilePictureBase64.subscribe(myData => {
                this.imgApplicant = myData;
            });
         
          }, () => {
              this.imgApplicant = 'data:image/jpg;base64,';
          });
    
      }, error => {
          console.log('[ERROR - Fail to get applicant result] ' + error);
          this.loadingResult = true;
      });
    }

    submitFinal() {  
      this.submittedF = true;  
      let i;
      let j = this.reportForm.get("rept")["controls"] as FormArray;
      
      if (this.reportForm.valid){
        this.loadingFinal = true;
        // save success
        for (i = 0; i < j.length; i++) {
          console.log (this.reportForm.value.rept[i].candidateID)
          let data = {                  
            appID: this.reportForm.value.rept[i].candidateID,
            // intvwFnlScore: this.reportForm.value.rept[i].score,
            intvwFnlScore: this.dataCandidate[i].ttlmarks,
            intvwFnlResult: this.reportForm.value.rept[i].result,
            intvwFnlRemarks: this.reportForm.value.rept[i].remark,
          }

          let addCr = this._POST_api_Service.POST_REC_data('/recruitment/user/updateFnlRslt', data);
          let dataJUDel: any = {};
          addCr.subscribe(dataRes => {
            dataJUDel = dataRes;                                                                                                   
          }, 
          error => {
              console.log('[ERROR - Submit Final Result]')
          }
        )         
        }  
        //end save success

        //save result
        let data3 = {                  
         adv_ID: this.dataSession[0].adv_id
        }

        let addCr3 = this._POST_api_Service.POST_REC_data('/recruitment/user/updateReport', data3);
        let dataJUDel3: any = {};
        addCr3.subscribe(dataRes3 => {
          dataJUDel3 = dataRes3;                            
          this.notifier.notify('success','Report has been successfully submitted');                                                                        
        }, 
        error => {
          this.notifier.notify('error','Fail to submit report');
            console.log('[ERROR - Submit Report]')
        }
        ).add(() => {
          //Called when operation is complete (both success and error)
          this.reset();
          this.getPanelist();
          this.getHeadList(); 
          this.getHistoryList();               
          //window.scrollTo({top: 0, behavior: 'smooth'});
        }); 
        //end save result
      
      } 
      else{
        this.scrollToError();
      }
  }

  // Submit Final for NE Promo
  submitFinalNE() {  
    this.submittedF = true;  
    let i;
    let j = this.reportForm.get("rept")["controls"] as FormArray;
    
    if (this.reportForm.valid){
      this.loadingFinal = true;
      // save success
      for (i = 0; i < j.length; i++) {
        console.log (this.reportForm.value.rept[i].candidateID)
        let data = {                  
          appID: this.reportForm.value.rept[i].candidateID,
          intvwFnlScore: this.dataCandidate[i].ivttlmarks_ne,
          intvwFnlResult: this.reportForm.value.rept[i].result,
          intvwFnlRemarks: this.reportForm.value.rept[i].remark,
        }

        let addCr = this._POST_api_Service.POST_REC_data('/recruitment/user/updateFnlRslt', data);
        let dataJUDel: any = {};
        addCr.subscribe(dataRes => {
          dataJUDel = dataRes;                                                                                                   
        }, 
        error => {
            console.log('[ERROR - Submit Final Result]')
        }
      )         
      }  
      //end save success

      //save result
      let data3 = {                  
       adv_ID: this.dataSession[0].adv_id
      }

      let addCr3 = this._POST_api_Service.POST_REC_data('/recruitment/user/updateReport', data3);
      let dataJUDel3: any = {};
      addCr3.subscribe(dataRes3 => {
        dataJUDel3 = dataRes3;                            
        this.notifier.notify('success','Report has been successfully submitted');                                                                        
      }, 
      error => {
        this.notifier.notify('error','Fail to submit report');
          console.log('[ERROR - Submit Report]')
      }
      ).add(() => {
        //Called when operation is complete (both success and error)
        this.reset();
        this.getPanelist();
        this.getHeadList(); 
        this.getHistoryList();               
        //window.scrollTo({top: 0, behavior: 'smooth'});
      }); 
      //end save result
    
    } 
    else{
      this.scrollToError();
    }
}


  numericOnly(event): boolean {
      // noinspection JSDeprecatedSymbols
      const charCode = (event.which) ? event.which : event.key || event.keyCode;  // keyCode is deprecated but needed for some browsers
      return !(charCode === 101 || charCode === 69 || charCode === 45 || charCode === 43);
  }


 /** START: SELECT APPLICANT FOR INTERVIEW */
 applInfoForm: FormGroup;
 applName: string;
 applSelType: string;
 applId: string; applStatus: string;
 applIndex: string;
 applLoading: boolean;
 applProfile: object;
 applCriteriaPromo: object;
 applAssessment: object;
 applHistory: object;
 jobsApply: object;
 applDownloadFlag: boolean;
 applDownloadFlag2: boolean;
 applResumeFlag = true;
 opnApplType: string;

 // Content for PDF
 titlePdf: String;
 docDefinition; 
 resumeUrl;
 AssessmentResFCAPI = INVars.AssessmentResFCAPI;
 AssessmentResTCAPI = INVars.AssessmentResTCAPI;
 FCResult;
 TCResult;
 tempDataQuaRes;
 filterResult1: any;
 urlTemp;
 urlHold;
 

    getUserCP(itemid){
     
      let data = {
         id: itemid
     }
     let applDetailsSend = this._POST_api_Service.POST_data(INVars.getApplicantDetailsAPI, data);
 
     this.applLoading = true;
     this.applName = `Applicant Information - ${itemid}`;
     this.applDownloadFlag = true;
     this.applDownloadFlag2 = true;
  
     let res = applDetailsSend.subscribe(dataQuaRes => {
         this.tempDataQuaRes = dataQuaRes;
         // Try to get more information about the requestor from localStorage 
         try {
             this.userId = JSON.parse(localStorage.getItem('currentUser')).userid;
         } catch (e) {
             console.error("Failed to get localStorage for currentUser");
         }
        
         this.theDate = this.datePipe.transform(new Date(), 'dd-MM-yyyy h:mm a');

         // Try to invoke the API to get personnel profile image
         let theUrl = `${INVars.APIGetImg}/${dataQuaRes.profile[0].image_url}`;
        
         let profilePictureSend = this._GET_api_Service.GET_Picture(theUrl);

         profilePictureSend.subscribe(pictureResults => {
             this.imgDataUrl = '';
             let profilePictureBase64 = this._GET_api_Service.GET_Base64(pictureResults);
             profilePictureBase64.subscribe(myData => {
                 this.imgDataUrl = myData;
             });
         }, () => {
             this.imgDataUrl = 'data:image/jpg;base64,';
         });

         this.applProfile = dataQuaRes.profile[0];
         this.applCriteriaPromo = dataQuaRes.criteria;
         this.applAssessment = dataQuaRes.assessment;
         this.resumeUrl = dataQuaRes.profile[0].resume;
         if(this.resumeUrl){
             this.applResumeFlag = false;
         }

         this.applHistory = dataQuaRes.history;
         this.filterResult1 = dataQuaRes.jobsApply.filter(u=> u.id == this.urlHold );
         
         this.jobsApply = this.filterResult1;

         this.applName = `Applicant Information - ${dataQuaRes.profile[0].Name}`;
        this.applId = itemid;
       
         // Set the title
         this.titlePdf = `Career Profile - ${dataQuaRes.profile[0].Staff_No}.pdf`;
        
         setTimeout(() => {

             let user = this.userId;
             let profile_img = this.imgDataUrl;
             this.docDefinition = {
                 pageSize: 'A4',
                 pageMargins: [20, 90],
                 watermark: { text: `By: ${this.userId}@${this.theDate}`, color: '#e0e0d1', opacity: 0.3, bold: true },
                 background: function(page) {
                     if (page !== 1) {
                         return [
                             {
                                 columns: [
                                     {
                                         width: 175,
                                         alignment: 'center',
                                         table: {
                                             width: ['auto'],
                                             body: [
                                                 [{ image: profile_img, width: 95, height: 95 }],
                                                 [{ text: `\n` }],
                                                 [{ text: `${dataQuaRes.profile[0].Name}`, style: 'profile_name' }],
                                                 [{ text: `${dataQuaRes.profile[0].Post_Desc}`, style: 'blackSize10' }],
                                                 [{ text: `${dataQuaRes.profile[0].Company_Desc}`, style: 'blackSize10' }],
                                                 [{ columns: [{ text: 'Pers. No ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Pers_no}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Staff ID ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Staff_No}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Birth Date ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Birth_Date}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Age ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Age}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Emp. Group ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].EmpGroup}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Years in Service ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YIS_HireDt}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Years in Band ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YIB}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Salary Grade ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Job_Grad}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'YTM scholar ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YTM_Scholar}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Talent ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Talent}`, style: 'blackSize10' },] }],
                                                 [{ columns: [{ text: 'Flight Risk ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].flight_risk}`, style: 'blackSize10' },] }],

                                             ]
                                         },
                                         layout: 'noBorders',
                                         margin: [20, 105, 0, 0]
                                     },
                                     {

                                     }
                                 ],
                             }
                         ];
                     }
                 },
                 header: {},
                 footer: function(currentPage, pageCount) {
                     return {
                       
                         text: 'Page : ' + currentPage.toString() + ' / ' + pageCount,
                         color: 'gray',
                         bold: 'true',
                         alignment: 'right',
                         fontSize: 11,
                         margin: 20
                     };
                 },
                 content: [],
                 images: {
                     logoEra: 'data:image/png;base64,',
                     logoTM: 'data:image/png;base64,'
                 },
                 styles: {
                     era_title: {
                         color: '#ea1819',
                         fontSize: 20,
                         bold: true,
                         alignment: 'center',
                         margin: [0, 20, 0, 0]

                     },
                     profile_name: {
                         bold: true,
                         color: '#fd5806',
                         fontSize: 12,
                         alignment: 'left',
                     },
                     header: {
                         color: '#fd5806',
                         bold: true,
                         fontSize: 14,
                     },
                     profile_header: {
                         color: '#5e6063',
                         bold: true,
                         fontSize: 12,
                         alignment: 'left',
                     },
                     wm_title: {
                         bold: true,
                         fontSize: 30,
                         color: '#5e6063',
                         opacity: 0.3
                     },
                     wm_staffId: {
                         bold: true,
                         fontSize: 40,
                         color: '#5e6063',
                         opacity: 0.3
                     },
                     lst_data: {
                         bold: true,
                         fontSize: 11,
                     },
                     lst_data_color: {
                         bold: true,
                         fontSize: 11,
                         color: '#fd5806'
                     },
                     red10: {
                         fontSize: 10,
                         color: 'red',
                         bold: true
                     },
                     lst_title_no: {
                         fontSize: 11,
                         bold: false,
                         color: '#5c6066',
                     },
                     lst_title: {
                         fontSize: 11,
                         bold: false,
                         color: '#5c6066',
                         margin: [20, 0, 0, 0]

                     },
                     bold: {
                         bold: true
                     },
                     greySize10: {
                         fontSize: 10,
                         color: '#5e6063',
                         bold: true,
                         alignment: 'left'
                     },
                     blackSize10: {
                         fontSize: 10,
                         bold: true,
                         alignment: 'left'
                     }
                 }
             };

             // Populate the header of the PDF 
             this.docDefinition.header = {
                 table: {
                     widths: ['auto', '*', 'auto'],
                     //headerRows: 1,
                     body: [
                         [{ rowSpan: 3, image: 'logoEra', fit: [80, 80], margin: [0, 7, 0, 10], bold: true }, { rowSpan: 3, text: `EMPLOYEE CAREER PROFILE`, style: 'era_title' }, { rowSpan: 3, image: 'logoTM', fit: [100, 100] }],
                         [{ text: `` }, '', ''],
                         [{ text: ``, margin: [0, 0, 0, 10], bold: true }, '', ''],
                         [{ colSpan: 3, text: '', fillColor: '#ff3300' }]
                     ]
                 },
                 layout: 'noBorders',
                 margin: [20, 20, 20, 40]
             };


             // Populate the content of PDF
             let myContent;
             this.applDownloadFlag = false;
             this.docDefinition.content = [];

            myContent = [
                 {
                     // 0.
                     table: {
                         widths: [350],
                         body: [
                             [{ text: '' }]
                         ]
                     },
                     layout: 'noBorders',
                     margin: [0, 20, 0, 5]
                 },
                 {
                     // 1.

                     columns: [
                         {
                             width: 165,
                             alignment: 'center',
                             table: {
                                 width: ['auto'],
                                 body: [
                                     [{ image: this.imgDataUrl, width: 95, height: 95 }],
                                     [{ text: `\n` }],
                                     [{ text: `${dataQuaRes.profile[0].Name}`, style: 'profile_name' }],
                                     [{ text: `${dataQuaRes.profile[0].Post_Desc}`, style: 'blackSize10' }],
                                     [{ text: `${dataQuaRes.profile[0].Company_Desc}`, style: 'blackSize10' }],
                                     [{ columns: [{ text: 'Pers. No ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Pers_no}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Staff ID ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Staff_No}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Birth Date ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Birth_Date}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Age ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Age}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Emp. Group ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].EmpGroup}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Years in Service ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YIS_HireDt}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Years in Band ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YIB}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Salary Grade ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Job_Grad}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'YTM scholar ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].YTM_Scholar}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Talent ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].Talent}`, style: 'blackSize10' },] }],
                                     [{ columns: [{ text: 'Flight Risk ', style: 'greySize10' }, { width: 5, text: ':', style: 'blackSize10' }, { text: `${dataQuaRes.profile[0].flight_risk}`, style: 'blackSize10' },] }],

                                 ]
                             },
                             layout: 'noBorders',
                         },
                         {
                             width: 'auto', margin: 5,
                             type: 'none',
                             ul:
                                 [
                                     {   //ul[0]
                                         table: {
                                             body: [
                                                 [{ text: 'JOB EXPERIENCE', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[1]
                                         text: '\n'
                                     },
                                     {   //ul[2]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[3]
                                         text: '\n\n'
                                     },

                                     {   //ul[4]
                                         table: {
                                             body: [
                                                 [{ text: 'PREVIOUS EMPLOYMENT', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[5]
                                         text: '\n'
                                     },
                                     {   //ul[6]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[7]
                                         text: '\n\n'
                                     },

                                     {   //ul[8]
                                         table: {
                                             body: [
                                                 [{ text: 'EDUCATION BACKGROUND', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[9]
                                         text: '\n'
                                     },
                                     {   //ul[10]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[11]
                                         text: '\n\n'
                                     },

                                     {   //ul[12]
                                         table: {
                                             body: [
                                                 [{ text: 'PROFESSIONAL QUALIFICATION', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[13]
                                         text: '\n'
                                     },
                                     {   //ul[14]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[15]
                                         text: '\n\n'
                                     },

                                     {   //ul[16]
                                         table: {
                                             body: [
                                                 [{ text: 'AWARDS / MERIT', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[17]
                                         text: '\n'
                                     },
                                     {   //ul[18]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[19]
                                         text: '\n\n'
                                     },

                                     {   //ul[20]
                                         table: {
                                             body: [
                                                 [{ text: 'PERFORMANCE APPRAISAL', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[21]
                                         text: '\n'
                                     },
                                     {   //ul[22]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[23]
                                         text: '\n\n'
                                     },

                                     {   //ul[24]
                                         table: {
                                             body: [
                                                 [{ text: 'UPGRADING AND PROMOTION', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[25]
                                         text: '\n'
                                     },
                                     {   //ul[26]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[27]
                                         text: '\n\n'
                                     },

                                     {   //ul[28]
                                         table: {
                                             body: [
                                                 [{ text: 'ASSESSMENT RESULT', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[29]
                                         text: '\n'
                                     },
                                     {   //ul[30]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },

                                     {   //ul[31]
                                         text: '\n\n'
                                     },

                                     {   //ul[32]
                                         table: {
                                             body: [
                                                 [{ text: 'SKILLSET MATCHUP', style: 'header' }]
                                             ]
                                         }, layout: 'noBorders',
                                     },
                                     {   //ul[33]
                                         text: '\n'
                                     },
                                     {   //ul[34]
                                         type: 'none',
                                         ul: [
                                             { text: [{ text: `#.    `, style: 'red10' }, { text: 'NIL\n', style: 'lst_title' }] }
                                         ]
                                     },
                                 ],

                         },

                     ]
                 },

             ];

             // Populate - JOB EXPERIENCE (ul[2])
             if (dataQuaRes.career.exprience.length) {
                 let exprienceList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.exprience.forEach(function(myVal) {
                     let myRow = [];
                     var n = (num < 10) ? ('0' + num++) : num++;
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Position ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Post_Desc}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Division/Unit ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Org_Unit_Desc}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Duration ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date} - ${myVal.End_Date}`, style: 'lst_data' }] });
                     if (myVal.Achievements)
                         myRow.push({ columns: [{ width: 100, text: 'Achievements ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Achievements}`, style: 'lst_data' }] });
                     else
                         myRow.push({ columns: [{ width: 100, text: 'Achievements ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `N/A`, style: 'lst_data' }] });

                     myRow.push({ text: '\n' });

                     exprienceList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[2] = exprienceList;
             }

             // Populate - PREVIOUS EMPLOYMENT (ul[6])
             if (dataQuaRes.career.previous.length) {
                 let previousEmpList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.previous.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Employer ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Employer}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Duration ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date} - ${myVal.End_Date}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     previousEmpList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[6] = previousEmpList;
             }

             // Populate - EDUCATION BACKGROUND (ul[10])
             if (dataQuaRes.career.education.length) {
                 let eduBackList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.education.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Institution ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.University}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Qualification ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Qualification_Desc}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Branch of Study ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Branch_of_Study}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Certificate ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Certificate}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     eduBackList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[10] = eduBackList;
             }

             // Populate - PROFESSIONAL QUALIFICATION (ul[14])
             if (dataQuaRes.career.profCert.length) {
                 let profQualList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.profCert.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Qualification ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Qualification}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Duration ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.Start_Date} - ${myVal.End_Date}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     profQualList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[14] = profQualList;
             }

             // Populate - AWARDS/MERITS (ul[18])
             if (dataQuaRes.career.award.length) {
                 let awardList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.award.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Award ' }], style: 'lst_title_no' }, { width: 5, text: ':', style: 'lst_data' }, { text: `${myVal.Award}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Awarded On ', style: 'lst_title' }, { width: 5, text: ':', style: 'lst_data' }, { text: `${myVal.Start_Date}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     awardList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[18] = awardList;
             }

             // Populate - PERFORMANCE APPRAISAL (ul[22])
             if (dataQuaRes.career.performance.length) {
                 let performList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.performance.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Year ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.years}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Rating ', style: 'lst_title' }, { text: [{ text: `: `, style: 'lst_data' }, { text: `${myVal.rating} `, style: 'lst_data_color' }, { text: `(${myVal.description})`, style: 'lst_data' }] }] });
                     myRow.push({ text: '\n' });

                     performList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[22] = performList;
             }

             // Populate - UPGRADING AND PROMOTION (ul[26])


             // Populate - ASSESSMENT RESULT (ul[30])
             if (dataQuaRes.career.assessment.length) {
                 let assessmentList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.assessment.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 100, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Year ' }], style: 'lst_title_no' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.years}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Assessment ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.assessment}`, style: 'lst_data' }] });
                     myRow.push({ columns: [{ width: 100, text: 'Rating ', style: 'lst_title' }, { width: 5, text: `:`, style: 'lst_data' }, { text: `${myVal.rating}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     assessmentList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[30] = assessmentList;
             }

             // Populate - SKILLSET MATCHUP (ul[34])
             if (dataQuaRes.career.skillset.length) {
                 let skillsetList = {
                     type: 'none',
                     ul: [

                     ]
                 };
                 let num = 1;
                 dataQuaRes.career.skillset.forEach(function(myVal) {
                     var n = (num < 10) ? ('0' + num++) : num++;
                     let myRow = [];
                     myRow.push({ columns: [{ width: 120, text: [{ text: `${n}.  `, style: 'red10' }, { text: 'Skill | Endorsements ' }], style: 'lst_title_no' }, { text: [{ text: `: ${myVal.skillset} `, style: 'lst_data' }, { text: `| ${myVal.skill_endorsement}`, style: 'lst_data_color' }] }] });
                     myRow.push({ columns: [{ width: 120, text: 'Skill Level ', style: 'lst_title' }, { text: `: ${myVal.skill_level}`, style: 'lst_data' }] });
                     myRow.push({ text: '\n' });

                     skillsetList.ul.push(myRow);
                 });

                 myContent[1].columns[1].ul[34] = skillsetList;
             }

             this.docDefinition.content.push(myContent);
             this.downloadCP1Form();
           
         }, 1500);

      
     }, error => {
         console.error("Failed to get information for user " + itemid);
     });
 }

  downloadCP1Form() {
    pdfMake.createPdf(this.docDefinition).download(this.titlePdf);
    this.imgDataUrl = '';  
  }

     
     // checkbox for Declare Integrity
      checkBtn(event) {
        this.ischeck = event.target.checked;
       }

       resetAddBtn(){
        this.ischeck = false;
       }

    //Panel Accept Declaration
  //  addPnLoading = false;
   panelAcceptForm(sessId){ 
     let dataPnAccept: any = {};
   
      dataPnAccept = {
      sessionID: sessId,
     }
    
    //  this.addPnLoading = true; 
     this.loading = true; 
    
     let pnlSend = this._POST_api_Service.POST_REC_data(INVars.postPanelAccept, dataPnAccept);
     let dataAcc: any = {};
     let ret = pnlSend.subscribe(dataRes => {
      dataAcc = dataRes;
     
         if (dataAcc.status === "OK") {
           this.notifier.notify('success', 'Successfully Accepted!');
           this.getPanelist();
          } else {
           this.notifier.notify('error', 'Error - Fail to Accept!');
       }
         this.loading = false; 
        //  this.addPnLoading = false; 
        },
         error => {
             console.log('[ERROR + User Not Found: ' + error);
        }) 
   }
       

}
