...
@Component({
    selector: 'eletter-tracking',
    templateUrl: './tracking.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['./tracking.component.css']
})

export class ELetterTrackingComponent implements OnInit {
  ...
  view_teamApproval = false;
  view_selfTracking = true;

  basicInfo;
  viewset;
  approval_movementPermitForm1: FormGroup;
  approval_movementPermitForm2: FormGroup;
  approval_movementPermitForm3: FormGroup;
  currDate = new Date();
  ...
  ngOnInit() {
    ...
    // this.getPurpose();
    // this.getSupervisor();
    this.getApprovalLetterList();
    
    this.route.queryParams
      .subscribe(params => {
        this.viewset = params.view;
      }
    );
    if(this.viewset=='self'){      
      this.view_teamApproval = false;
      this.view_selfTracking = true;
    }
  ...
  getBasicInfo(){
    this.loading = true; 
    this._GET_api_Service.GET_EVL_data(trackingVars.apiGetBasicInfo).subscribe(data => {
      ...
      console.log('basicinfo: ',data);
          ...
          console.log('[ERROR] API: ',trackingVars.apiGetBasicInfo,'ErrMsg: ',error);
    })
    this.loading = false; 
  }

  appliedLetterListLatestSuccess;
  appliedLetterList=[];
  getLetterList(){
    this.loading = true; 
    this._GET_api_Service.GET_EVL_data(trackingVars.apiGetAppliedLetterList).subscribe(data => {
      this.appliedLetterList=data;
      console.log('appliedLetterList: ',data);
      // this.appliedLetterListLatestSuccess = data.filter(
      //   item => (item.status == '6' || item.status == '7')
      // );
    }, error => {
          console.log('[ERROR] API: ',trackingVars.apiGetAppliedLetterList,'ErrMsg: ',error);
    })
    this.loading = false; 

  }

  checkAvaibilityDw(xdate){
    let newDate = new Date(xdate);
    let currDateAddDay = new Date(this.currDate.getDate() + parseInt('1'));
    console.log('newDate.getDate',newDate,'currDateAddDay.getDate',currDateAddDay);
    if(newDate.getDate < currDateAddDay.getDate){
      return false;
    } else {
      return true;
    }
  ...
  approvalLetterList=[];
  getApprovalLetterList(){
    this.loading = true; 
    this._GET_api_Service.GET_EVL_data(trackingVars.apiGetApprovalList).subscribe(data => {
      this.approvalLetterList=data;
      console.log('approvalLetterList: ',data);
    }, error => {
          console.log('[ERROR] API: ',trackingVars.apiGetApprovalList,'ErrMsg: ',error);
    })
    this.loading = false; 
  }
  ...
  getPurpose() {
      this.loading = true; 
      this._GET_api_Service.GET_EVL_data(trackingVars.apiGetPurpose).subscribe(data => {
        this.purposeList=data;
        console.log('purposeList',data)
      }, error => {
            console.log('[ERROR] API: ',trackingVars.apiGetPurpose,'ErrMsg: ',error);
      })
      this.loading = false; 
  }
  ...
  getSupervisor() {
      this.loading = true; 
      this._GET_api_Service.GET_EVL_data(trackingVars.apiGetSupervisor).subscribe(data => {
        this.sup_name=data[0].Name;
        this.sup_staff_No=data[0].staff_No;
        this.sup_Post_Desc=data[0].Post_Desc;
        console.log('supervisor',data)
      }, error => {
            console.log('[ERROR] API: ',trackingVars.apiGetSupervisor,'ErrMsg: ',error);
      })
      this.loading = false; 
  }
  ...
  typeid_descr;
  checkStatusDescByLTypeId(ltype_id, ltype_stats){
    switch(ltype_id) { 
      case '1': { 
         //statements; 
         if(ltype_stats=='1'){
           this.typeid_descr = 'Apply';
           return true;
         } else if (ltype_stats=='7'){
           this.typeid_descr = 'Success';
           return true;
         }
         break; 
      } 
  ...
  getAttachment(dataId: any) {  
    this._GET_api_Service.GET_EVL_data(trackingVars.apiGetEndorsedDocumentLists+dataId).subscribe(data => {
        if(data){
          this.attachList = data
          console.log('dataAttachemnt',data);
        } else {
          this.attachList = [];  
        };    
    });
  ...
  loadDetail(xl_type,xl_id){
    if(xl_type=='4'){
        this.getAttachment(xl_id);
    }
  }
  ...
  getLetterTypeList() {
    ...
    this._GET_api_Service
      .GET_EVL_data(trackingVars.apiGetAppliedLetterTypeList)
      .subscribe(
        (data) => {
        ...
        (error) => {
          console.log(
            "[ERROR] API: ",
            trackingVars.apiGetAppliedLetterTypeList,
            "ErrMsg: ",
            error
          );
        }
  ...
  selectLetterType(ltid: string) {
    ...
    if(ltid){
      ...
      this._GET_api_Service.GET_EVL_data(trackingVars.apiGetActiveByLttrType+ltid).subscribe(data => {      
        console.log('apiGetActiveByLttrType',data)
            this.selectedLetterActiveStatus=data.results;
        }, error => {
              console.log('[ERROR] API: ',trackingVars.apiGetActiveByLttrType,'ErrMsg: ',error);
      })
  ...
  setLttrHeader(){
    ...
    console.log('lttr_comp_header',this.lttr_comp_header);
  }

  selectedLetterById;
  checkActiveDownload(lttrid){    
    this.loading = true; 

    if(this.selectedLetterById != lttrid){
      this.selectedLetterById = lttrid;
      this.selectedLetterActiveStatus = false;

      let apiLttrDetail = trackingVars.apiGetLetterById + lttrid;
      this._GET_api_Service.GET_EVL_data(apiLttrDetail).subscribe(data => {
        let lttr_detail=data[0];
        console.log('lttr_detail',lttr_detail);
          if(lttr_detail.updated_on && (this.datePipe.transform(lttr_detail.updated_on, 'yyyyMMdd') == this.datePipe.transform(this.currDate, 'yyyyMMdd'))){
            this.selectedLetterActiveStatus = true;
          }
        console.log('selectedLetterById',this.selectedLetterById);
        console.log('selectedLetterActiveStatus',this.selectedLetterActiveStatus);
      }, error => {
            console.log('[ERROR] API: ',apiLttrDetail,'ErrMsg: ',error);
      })
    }
  ...
  loadLttrById(lttrid){    
    ...
    let apiLttrDetail = trackingVars.apiGetLetterById + lttrid;
    this._GET_api_Service.GET_EVL_data(apiLttrDetail).subscribe(data => {
      this.lttr_detail=data[0];
      let lttr_detail=data[0];
      console.log('lttr_info',data[0])

      //if data exist
      if(lttr_detail){      
        let lttr_type_id= lttr_detail.ltype;
        //console.log('lttr_type_id',lttr_type_id);
        this.selectLetterType(lttr_type_id);
        switch (lttr_type_id) {
          //tmhousing loan
          case 1:
            this.genLetterTmHousingLoan();
            break;
          //other financial
          case 2:
            
            this.lttr_bank_t2_name = lttr_detail.recipient_name;
            this.lttr_bank_t2_address1 = lttr_detail.address_1;
            this.lttr_bank_t2_address2 = lttr_detail.address_2;
            this.lttr_bank_t2_address3 = lttr_detail.address_3;
            this.genLetterHousingLoan();
            break;
          default:
  ...
  dw_loadingModal(){
    ...
    var btnId = this.selectedLetterById;
    //disable button generate offer and countdown 10 sec to prevent multiple attempt
    $('#btn-dw1-'+btnId).prop('disabled', true);
    $('#btn-dw1-'+btnId).addClass("disabled");
    // $('#btn-dw1').hide();
    var timeleft = 10;
    ...
    if(timeleft <= 0){
        ...
        $('#btn-dw1-'+btnId).prop('disabled', false);
        $('#btn-dw1-'+btnId).removeClass("disabled");
    } else {
