...
export class UserProfileComponent implements OnInit {
    ...
    nickName;
    getNickName() {
        let api = '/sea/getNickname'
        this._GET_api_Service.GET_SEA_data(api).subscribe(data => {
            this.nickName = data[0].nick_name;
        },error => {
            console.log('[ERROR Get Nick name] ' + error);
        });
    }

    isNickEdit = false;
    editNickName() {
        this.isNickEdit = true;
    }

    loadingNickName = false;
    saveNickName() {
        this.loadingNickName = true;
        let api = '/sea/editNickname';
        let posdata = {
            staffId: this.profile.selfID,
            name: this.nickName
        }
        //IDP and SEA api same
        this._POST_api_Service.POST_IDP_data(api, posdata).subscribe(data => {
            this.currentUser.body.nick_name = posdata.name;
            localStorage.setItem('currentUser', JSON.stringify(this.currentUser));

            this.loadingNickName = false;
            this.isNickEdit = false;
        }, err => {
            this.loadingNickName = false;
            this.isNickEdit = false;
            console.log('[ERROR update Nick name] ' + err);
        })
    }
    ...
    dpChange(event) {
        ...
        this._POST_api_Service.POST_ScreenShot(this.uploadImage, form_Data).subscribe(res => {
            ...
            if(res.results ===  true){
                this.imgOptArrList = ""
                this.currentUser.body.image_url = res.image_url;
                localStorage.setItem('currentUser', JSON.stringify(this.currentUser));
                this.getImgOpt();
            }
        })
    ...
    ngAfterViewInit() {
        this._script.loadScripts('app-u-profile',
            [
