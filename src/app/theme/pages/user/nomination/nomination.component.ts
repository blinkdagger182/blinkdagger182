...
import { NominationVars, LOB, lobArr, Successpr } from './nomination-vars';
...
import { CircleVars } from '../circle/circle-vars';
import { recfriend, recFriendArr, allfriendArr, skillArr, profileInfoArr } from "../circle/arrayCons";
...
@Component({
    selector: 'app-u-nomination',
    templateUrl: 'nomination.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['nomination.css'],
    
})


export class nominationComponent implements OnInit, AfterViewInit {
@ViewChild('profileLnk') profileLnk: ElementRef;
@ViewChild('modalflightbtn') flightRiskModalBtn: ElementRef;
@ViewChild('modalconfirmbtn') confirmBtn: ElementRef;
@ViewChild('img') image: ElementRef;

    APIallFriend = CircleVars.APIallFriend;
    APIgetSubordinates = NominationVars.APIgetSubordinates;
    APIgetUserProfile = NominationVars.APIgetUserProfile;
    ...
    spNominationUpd = NominationVars.spNominationUpd;
    userList = [];
    options: IOption[];
    selected: IOption[];
    ...
    checkLevel() {
        ...
        this._GET_api_Service.GET_data(NominationVars.actionSummaryAPI).subscribe(sum => {
    ...
    updStatusSubmit() {
        ...
        let countOverall = Number(this.countSuccessType4 + this.countSuccessType3 + this.countSuccessType2); 

        if(countOverall == null || countOverall < 1)
        {
            let inputElement: HTMLElement = this.confirmBtn.nativeElement as HTMLElement;
            inputElement.click();
        }
        else{
            this.confirm();
    ...
    confirm(){
        ...
        let updStatusSubmitSend = this._POST_api_Service.POST_SP_data(this.spNominationUpd, dataPost);
        let dataSPRem: any = {};
        let ret = updStatusSubmitSend.subscribe(dataRes => {
            dataSPRem = dataRes;
            if (dataSPRem.message === "Success") {
                this.notifier.notify('success', 'Successfully submit the nomination!');
                this.getSpBatchDetails();
                this.managerClick(this.profile);
                this.triggerFalseClick();
            } else {
                this.notifier.notify('error', 'Error - Cannot submit the nomination !');
            }
    ...
    getSpBatchDetail() {
        ...
        this._GET_api_Service.GET_SP_DATA(this.getSPCurrBatch).subscribe(res => {
            SpBatchData = res;
            let expiredVal: number;
            this.currBatchId = SpBatchData[0].batch_id;
            expiredVal = SpBatchData[0].Expired;

            this.isExpired = expiredVal > 0 ? true : false;
    ...
    getProfile() {
        ...
        this._GET_api_Service.GET_data(this.APIgetUserProfile).subscribe(data => {
            ...
            this.profile =  data.career.profile[0];
            //console.log('Profile', data);
            this.postId = this.profile.Position;
...
managerClick(profile) {
    ...
    this.personalDetails(profile.Staff_No)
    ...
    staffDetail(subUser) {
        ...
        if(this.successorID) {
            this.personalDetails(subUser.staff_no)
    ...
    getSubordinates() {
        ...
        this._GET_api_Service.GET_SP_DATA(this.APIgetSubordinates).subscribe(data => {
    ...
    getSuccessorsRefresh() {
        ...
        this._GET_api_Service.GET_SP_DATA(this.APIgetSuccessors).subscribe(data => {
        ...
        {
            this.btnSubmit = "Re-Submit";
        }
        

        this.updateStatusCaption(captionStatus);
    ...
    updateStatusCaption(sts:string){

        
        if(sts === null || sts === '' || sts === undefined) this.statusCaption = 'Nomination'
        else this.statusCaption = sts;
